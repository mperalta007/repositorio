<?php
if(str_replace( '\\', '/',$_SERVER['DOCUMENT_ROOT']) == SERVER_ROOT) { define('ABS_PATH', '/include/jqGrid');}
else { define('ABS_PATH', '/include/jqGrid'); }
...
//    private $_file_path;
    ...
    private function display_style(){
        ...
        if(!empty($this->col_autocomplete)){
            echo '#select2-drop{font-family:arial;font-size:14px;}';
            echo '.select2-no-results{color:rgb(163, 163, 163);font-size:14px}';
        }
    ...
    public function display_script_includeonce(){
        if($this->sql_fkey==null){
            ...
            $this->script_includeonce .='<script type="text/javascript">
					if (typeof jQuery == "undefined"){document.write("<script src=\''. ABS_PATH .'/js/jquery-1.11.0.min.js\' type=\'text/javascript\'><\/script>");}
				  </script>' ."\n";
    ...
    public function display_properties_main(){
                    ...
                    function(xhr,status, err) {
                        try{
                            jQuery.jgrid.info_dialog(
                                ...
                                "<div style=\"font-size:14px;text-align:center;width:300px;;height:150px;overflow:auto;color:red;\">"+ xhr.responseText +"</div>",
        ...
        switch($this->edit_mode){
            ...
            case 'INLINE':
                ...
                if((strrpos($this->edit_options,"U")!==false)){
                    ...
                    echo '});' ."\n"; // grid.jqGrid("editRow"...
                    echo '$("input, select",e.target).focus();' ."\n";
        ...
        if(count($this->jq_cellConditions)>0 || count($this->jq_rowConditions)>0){
            ...
            for ($i=0;$i<count($this->jq_rowConditions);$i++){
                ...
                $metatype = $this->db->field_metatype($result, $colIndex);
                if($metatype == 'T' || $metatype == 'D'){
                    $rowStr.= "if (item.cell['$colIndex'] != null) {".$this->generate_condition_date($colIndex, $options["condition"],$options["value"]) ;
                }else{
    ...
    private function generate_condition_date($colIndex,$condition,$value)
    {
    ...
    private function display_subgrid(&$cnt){
        if($this->obj_subgrid != null){
            ...
            echo 'subGridRowExpanded: function(subgrid_id'. $cnt .', row_id'. $cnt .') {
                    var fkey_value, subgrid_table_id'. $cnt .', pager_id'. $cnt .';
                    ...
                    if($cnt > 1){
                        echo 'fkey_value = $("#"+subgrid_table_id'. ($cnt-1) .').jqGrid("getCell", row_id'. ($cnt) .', "'. $this->obj_subgrid->get_sql_fkey() .'");';
                        /*
                         echo '// console.log(subgrid_table_id'. ($cnt). ' + "  |  " + subgrid_id'. ($cnt). ' + "  |  " + row_id'. ($cnt) .');
                              // console.log(fkey_value);';
                        */
                    }else{
                        echo 'fkey_value = $("#'. $this->jq_gridName .'").jqGrid("getCell", row_id'. $cnt .', "'. $this->obj_subgrid->get_sql_fkey() .'");';
                    }
...
//        $this->db->db->debug = $debug;
    ...
    // set coulmn width
    public function set_col_align($col_name, $align="center"){
    ...
    private function get_nested_dropdown_js($col_name){
        foreach($this->col_nested_dd as $key => $value){
            ...
            if($col_name == $key){
                ...
                echo '$("#'. $this->jq_gridName .' tr#"+id+" td select[id="+id+"_'. $key .']").on("change", function() {
                            $("#'. $this->jq_gridName .' tr#"+id+" td select[id="+id+"_'. $col_d .']")
                                .select2("val", "")
                                .empty(); // clear the list first
